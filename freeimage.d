...
typedef uint function(void* buffer, uint size, uint count, fi_handle handle)
		FI_ReadProc;
typedef uint function(void* buffer, uint size, uint count, fi_handle handle)
		FI_WriteProc;
...
alias void function(FREE_IMAGE_FORMAT fif, char* msg)
...
extern(Windows) {
	...
	FIBITMAP* FreeImage_Allocate(int width, int height, int bpp,
			uint red_mask = 0, uint green_mask = 0, uint blue_mask = 0);

	FIBITMAP* FreeImage_AllocateT(FREE_IMAGE_TYPE type, int width, int height,
			int bpp = 8, uint red_mask = 0, uint green_mask = 0,
			uint blue_mask = 0);

	FIBITMAP* FreeImage_Load(FREE_IMAGE_FORMAT fif, char* filename,
			int flags = 0);

	BOOL FreeImage_Save(FREE_IMAGE_FORMAT fif, FIBITMAP* dib, char* filename,
			int flags = 0);
