...
GLint writeJpeg(string fileName)
{
  ...
  debug writefln("Image size: %s x %s", iViewport[2], iViewport[3]);
  ...
  glGetIntegerv(GL_READ_BUFFER, cast(int*)&lastBuffer);
  glReadBuffer(GL_FRONT);
  glReadPixels(0, 0, iViewport[2], iViewport[3], GL_BGR_EXT, GL_UNSIGNED_BYTE, pBits.ptr);
  glReadBuffer(lastBuffer);
  ...
  for(uint y=0; y<iViewport[3]; y++)
  {
    ...
    {
      FreeImage_SetPixelColor(thePicture, x, y, cast(RGBQUAD*)&pBits[x*3+3*y*iViewport[2]]);
    }
  }

  FreeImage_Save(FREE_IMAGE_FORMAT.FIF_JPEG,thePicture,cast(char*)(fileName~"\0"),JPEG_QUALITYSUPERB);
